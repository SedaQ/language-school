...
public class LectureDaoTest extends AbstractTestNGSpringContextTests {
	...
	// odtialto
	/*
	 * @Test public void testFindById() { lectureDao.create(l);
	 * Assert.assertEquals(l, lectureDao.findById(l.getId())); }
	 * 
	 * @Test public void testFindAll() { Lecture l1 = new Lecture();
	 * l1.setDayTime(LocalDateTime.MAX); l1.setTopic("Random topic");
	 * 
	 * lectureDao.create(l); lectureDao.create(l1);
	 * 
	 ...
	 * 
	 * @Test public void testCreateAndRemove() { lectureDao.create(l);
	 * Assert.assertEquals(lectureDao.findAll().size(), 1);
	 * lectureDao.remove(l); Assert.assertEquals(lectureDao.findAll().size(),
	 * 0); }
	 * 
	 * @Test public void testUpdate() { lectureDao.create(l); l.setTopic(
	 * "Something else"); lectureDao.update(l);
	 * Assert.assertEquals(l.getTopic(),
	 * lectureDao.findById(l.getId()).getTopic()); }
	 */
